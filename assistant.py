...
9.To show password of previsiously connected network''')
                            ...
                            if category== '1':
                                ...
                                speak(f"OK I will remember this and You can now use {self.command} command for terminating dev")
                                engine.runAndWait()
                                return 'exit'
                            elif category=='2':
                                ...
                                speak(f"Ok I will remember this and You can now use {self.command} command for introducing dev to yourself")
                                ...
                                speak(f"Ok I will remember this ,You can now use {self.command} command for greeting time from dev")
                                return 'todays_date'
                            elif category=='5':
                                ...
                                speak(f"Ok I will remember this ,You can now use {self.command} command to open your broswer")
                                return 'opening browser'
                            elif category=='7':
            ...
            if 'exit' == output:
                ...
                if browser_choice==1:
                    if url_choice.startswith('https:\\') or url_choice.startswith('http:\\'):
                        completed = subprocess.run(["powershell", "-Command",f'''[system.Diagnostics.Process]::Start('msedge','{url_choice}')''' ],
                                                   capture_output=True)
                    else:
...
Enter the name of your wifi: ''').strip()
                try:
                    ...
                    print("Password is =>",subprocess.check_output(['netsh', 'wlan', 'show', 'profiles', f'"{network_name}"', 'key=clear']).decode().replace('\r',
                                                                                                                  " ").split(
                         "\n")[32].split(':')[1].strip()) 
                except Exception:
                    print("Password is =>",subprocess.check_output(
